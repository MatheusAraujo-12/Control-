...
const OrdensDeServico = ({ userId, ordensDeServico = [], clients = [], services = [], professionals = [], setNotification }) => {
    ...
    useEffect(() => {
        ...
        return () => { unsubOS(); unsubClients(); unsubServices(); unsubPros(); unsubParts(); };
    }, [userId]);
    ...
    // Efeitos de Merge
    useEffect(() => setMergedOS(mergeLists(ordensDeServico, fetchedOS)), [ordensDeServico, fetchedOS]);
        ...
        try {
            ...
            if (savedOsId && osToSave.type === 'OS') {
                try {
                    ...
                    console.error("Erro integração financeira:", err);
                    setNotification({ type: 'warning', message: 'OS salva, mas erro ao sincronizar financeiro.' });
                }
            }
    ...
    const filteredOS = mergedOS.filter(os => {
        if (os.type === 'Orcamento') return false; // Excluir Orçamentos
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <Button onClick={() => { setCurrentOs(null); setIsModalOpen(true); }} icon={<Plus size={18} />}>Novo Lançamento</Button>
            </div>
